...
const CODE_OBJECT_HEADER_SIZE = 24
...
class CodeObject
    public initialize file_name = do
        this.string_registry = {}
        this.next_string_index = CODE_OBJECT_HEADER_SIZE + file_name.length()
        this.functions = []
        this.main_code = []
        this.loop_stack = []
    ...
    public serialize() = do
        ...
        ba.wrappedSetInteger(12, CODE_OBJECT_HEADER_SIZE + this.file_name.length())
        ba.wrappedSetInteger(16, code_offset)
        ba.wrappedSetInteger(20, entry_point)

        # write the file name
        for i in this.file_name.length()
            ba->(CODE_OBJECT_HEADER_SIZE + i) = this.file_name.charAtPosition i
        end
...
public disassemble ba = do
    ...
    private filename_offset = ba.getInteger 8
    private string_offset = ba.getInteger 12
    private code_offset = ba.getInteger 16
    private entry_point = ba.getInteger 20
    ...
    sb << "\tsize=" << size << "\n"
    sb << "\tfilename_offset=" << filename_offset << "\n"
...
public compile code file_name = do
    co = CodeObject file_name
    compile_rec code co
...
private compile_rec code co = do
    ...
    match code
        ...
        case Node.T_AND(@left, @right)
            ...
            compile_boolify co
            private end_label = Label()
            co.append(Opcode.JUMP_FALSE(0, end_label))
    ...
    for block in catch_blocks
        ...
        match block
            ...
            case Node.T_CATCH_IF(@guard, @body)
                compile_rec guard co
                co.append(Opcode.JUMP_FALSE(0, next_label))
                compile_rec body co
                co.append(Opcode.POP 3)
                co.append(Opcode.JUMP end_try_catch_label)
        end
