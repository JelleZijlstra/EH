...
EH_METHOD(VisibilityError, initialize) {
	...
	name->assert_type<String>("VisibilityError.initialize", ehi);
	obj->set_property("object", object, ehi->global(), ehi);
	obj->set_property("name", name, ehi->global(), ehi);
	std::ostringstream exception_msg;
	exception_msg << "Cannot access private member " << name->get<String>() << " in object of type " << ehi->get_parent()->repo.get_name(object);
