...
assert(A.B.isA(Enum.Instance), "member class")
...
assert(instance.type() == A, "it is an A")
...
# @method typeId
assert(A == A.B.type(), "typeId matches for all enum members and instances")
assert(A.B.typeId() == A.C(4).typeId(), "typeId matches (again)")
assert(A.B.typeId() != B.C.typeId(), "different enums have a different typeId")

# @method type
assert(A.type() == Enum, "type of A is A")
assert(A.B.type() == A, "type is A")
assert(A.C(3).type() == A, "type is still A")
assert(B.type() == Enum, "type is B")
